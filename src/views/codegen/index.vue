...
import { GeneralType } from "@/types/commonTypes";
...
async function handleOpenDialog(tableName: string) {
  ...
  // 获取字典数据
  DictAPI.getList().then((data: GeneralType) => {
    dictOptions.value = data.map((item: GeneralType) => ({
      label: item.name,
      value: item.dictCode,
    }));
    loading.value = true;
...
function handlePreview(tableName: string) {
  ...
  GeneratorAPI.getPreviewData(tableName)
    .then((data: { path: string; fileName: string; content: string }[]) => {
